 ...
 * loaded Act 1 scene set — either the natural end of Act 1 (handoff to
 * Act 2, not built yet) or an orphaned/unwritten branch in the source data.
 * The engine treats both the same way: stop cleanly instead of crashing.
 */
export default function EndOfContent({
  ...
  const isAct1Complete = reason === "act1-complete";

  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 bg-zinc-950 px-6 text-center">
      <div>
        <h2 className="text-2xl font-bold text-zinc-50">
          {isAct1Complete ? "Act 1 Complete" : "End of built content"}
        </h2>
        <p className="mt-2 max-w-md text-sm text-zinc-400">
          {isAct1Complete
            ? "The story continues into Act 2, which hasn't been built yet in this slice."
      ...
      <div className="w-full max-w-sm rounded-lg border border-zinc-800 bg-zinc-900 p-4 text-left text-xs">
        <div className="mb-2 font-semibold text-zinc-300">End-of-Act-1 state</div>
